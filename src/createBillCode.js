// PDF出力先
const BILL_PDF_OUTDIR = DriveApp.getFolderById("##ID##");
// 請求書のテンプレートファイル
const BILL_TEMPLATE = DriveApp.getFileById("##ID##");
function createBill(datas) {
  var data = JSON.parse(datas);
  var customerName = data["customerName"];
  var targetFrom = Utilities.formatDate(
    new Date(data["targetDateFrom"]),
    "JST",
    "yyyy/MM/dd"
  );
  var targetTo = Utilities.formatDate(
    new Date(data["targetDateTo"]),
    "JST",
    "yyyy/MM/dd"
  );
  ...
  if (customerName != "") {
    targetLists = items.reduce(function (result, target) {
      if (
        Utilities.formatDate(new Date(target["納品日"]), "JST", "yyyy/MM/dd") >=
          targetFrom &&
        Utilities.formatDate(new Date(target["納品日"]), "JST", "yyyy/MM/dd") <=
          targetTo &&
        ...
        if (result.length != 0) {
          const ele = result[0]["商品"].find(
            (value) => value["商品名"] === target["商品名"]
          );
          if (ele) {
...
function createBillGDoc(rowVal, billDate) {
  ...
  customerItems.forEach(function (wVal) {
    if (customerName.length > 1) {
      // 会社名と同じ
      if (
        customerName[1] == wVal["氏名"] &&
        customerName[0] == wVal["会社名"]
      ) {
        customerItem = wVal;
      }
    } else {
  ...
  wCopyDocBody = wCopyDocBody.replaceText(
    "{{delivery_date}}",
    Utilities.formatDate(new Date(billDate), "JST", "yyyy年MM月dd日")
  );
  ...
  wCopyDocBody = wCopyDocBody.replaceText(
    "{{depositDate}}",
    Utilities.formatDate(new Date(depositDate), "JST", "yyyy年MM月dd日")
  );
  let totals = 0;
  let tentax = 0;
  let eigtax = 0;
