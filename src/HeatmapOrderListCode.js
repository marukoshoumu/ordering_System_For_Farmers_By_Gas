 ...
 * 指定された期間内の日付リストを生成
 * 
 * @param {string} fromStr - 開始日 (yyyy-MM-dd)
 * @param {string} toStr - 終了日 (yyyy-MM-dd)
 * @returns {Array<string>} yyyy-MM-dd 形式の日付文字列の配列
 */
...
/**
 * 商品マスタ取得（略称列対応）
 * シート「商品」から「商品名」「略称」「商品分類」を取得します。
 * 
 * @param {Array<string>} categoryFilter - 絞り込み対象の商品分類の配列
 * @returns {Array<Object>} 商品情報の配列 [{fullName, shortName, category}]
 */
...
/**
 * ヒートマップ表示用にフィルタリングされた受注データを取得
 * 
 * @param {string} dateFrom - 開始日
 * @param {string} dateTo - 終了日
 * @param {Array<string>} categories - カテゴリフィルタ
 * @param {string} destination - 発送先フィルタキーワード
 * @param {string} customer - 顧客名フィルタキーワード
 * @returns {Array<Object>} 受注データの配列
 */
