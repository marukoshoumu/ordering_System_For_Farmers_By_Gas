...
function tabeAddRecord(data) {
  ...
  data.forEach(function (wVal) {
    ...
    const productData = productItems.find(
      (ele) => ele["商品名"] == wVal["商品名"]
    );

    const record = [
      wVal["注文番号"],
      Utilities.formatDate(
        new Date(tabeDateFormat(wVal["注文日"])),
        "JST",
        "yyyy/MM/dd"
      ),
      "",
      productData["変換値"],
      productData["変換数量"],
      Number(wVal["商品代金(お客様に提示した額)"]) +
        Number(wVal["生産者に支払われる送料"]) +
        Number(wVal["生産者に支払われるクール代"]),
      "サンプル",
      "999-9999",
      "サンプル",
      "",
      ...
      "9999-99-9999",
      Utilities.formatDate(
        new Date(tabeDateFormat(wVal["発送日"])),
        "JST",
        "yyyy/MM/dd"
      ),
      Utilities.formatDate(
        new Date(tabeDateFormat(wVal["お届け日"])),
        "JST",
        "yyyy/MM/dd"
      ),
      "",
...
function maruSheAddRecord(data) {
  ...
  data.forEach(function (wVal) {
    ...
    const productData = productItems.find(
      (ele) => ele["商品名"] == wVal["商品名"] && ele["分量"] == wVal["分量"]
    );
    const record = [
      wVal["注文ID"],
      Utilities.formatDate(
        maruDateFormat(wVal["注文確定日"]),
        "JST",
        "yyyy/MM/dd"
      ),
      "",
