...
### 商品マスタ一覧画面
- **紐づくシート**: 商品、商品分類、（freee連携プルダウン用）`freee勘定税区分`、`freee部門品目`
- **主な機能**:
  ...
  - **freee連携**: 勘定科目・税区分・部門・品目はスプレッドシートのマスタから読み込んだ選択肢のみ（コード埋め込みなし）。部門を選ぶと品目が絞り込まれる。新規時、マスタに「売上高」があれば勘定科目の初期値に使う。マスタ未整備時は画面上部に警告が出る。
...
#### freeeマスタシートの初回セットアップ（商品マスタと同一スプレッドシート）
1. Apps Script エディタを開き、関数 `initializeFreeeMasterSheets()` を実行する（コンテナバインドのスプレッドシートで実行すること）。
2. シート `freee勘定税区分`・`freee部門品目` が無ければ作成され、**データ行が1件も無いときだけ**、コード内の初期値（勘定・税区分・部門×品目の一覧）が自動投入される。既に手入力があるシートは上書きしない。
3. 初期値を変えたい場合はスプレッドシート上で直接編集する。投入内容は `initializeSpreadsheet.js` の `getFreeeAccountTaxSeedData` / `getFreeeDepartmentItemSeedData` を参照。
4. 商品マスタ画面を再読み込みする（マスタ変更後、最大約5分はキャッシュが残る場合がある。商品の保存などでマスタキャッシュは更新される）。
...
2. 「OAuth 同意画面」→ 公開ステータスを **「本番」** に変更（または Google Workspace なら「内部」）
   > **重要**: 「テスト」ステータスのままだとリフレッシュトークンが **7日で失効** します。必ず本番に公開してください。
...
5. リフレッシュトークンを取得:
   ...
   ```
6. Google Drive に以下のフォルダを作成し、フォルダIDをメモ:
...
**今後の予定**: Gemini APIによるエラースクリーンショット自動解析（段階2）、西濃運輸 Worker 自動取込（スコープ外）
